import FormContainer from "@/components/FormContainer";
import Pagination from "@/components/Pagination";
import Table from "@/components/Table";
import TableSearch from "@/components/TableSearch";
import prisma from "@/lib/prisma";
import { ITEM_PER_PAGE } from "@/lib/settings";
import { Prisma } from "@prisma/client";
import Image from "next/image";
import { auth } from "@clerk/nextjs/server";

const FeePaymentListPage = async ({
  searchParams,
}: {
  searchParams: { [key: string]: string | undefined };
}) => {
  const { userId, sessionClaims } = auth();
  const role = (sessionClaims?.metadata as { role?: string })?.role;
  const currentUserId = userId;

  const columns = [
    {
      header: "Student",
      accessor: "student",
    },
    {
      header: "Fee Type",
      accessor: "feeType",
    },
    {
      header: "Amount",
      accessor: "amount",
    },
    {
      header: "Status",
      accessor: "status",
    },
    {
      header: "Payment Method",
      accessor: "paymentMethod",
      className: "hidden md:table-cell",
    },
    {
      header: "Payment Date",
      accessor: "paymentDate",
      className: "hidden md:table-cell",
    },
    {
      header: "Receipt",
      accessor: "receiptNumber",
      className: "hidden md:table-cell",
    },
    {
      header: "Actions",
      accessor: "action",
    },
  ];

  const renderRow = (item: any) => (
    <tr
      key={item.id}
      className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight"
    >
      <td className="p-4">{item.student}</td>
      <td>{item.feeType}</td>
      <td>{item.amount}</td>
      <td>
        <span className={`px-2 py-1 rounded-full text-xs ${
          item.status === 'PAID' ? 'bg-green-100 text-green-800' :
          item.status === 'PENDING' ? 'bg-yellow-100 text-yellow-800' :
          item.status === 'FAILED' ? 'bg-red-100 text-red-800' :
          item.status === 'CANCELLED' ? 'bg-gray-100 text-gray-800' :
          'bg-blue-100 text-blue-800'
        }`}>
          {item.statusLabel}
        </span>
      </td>
      <td className="hidden md:table-cell">{item.paymentMethod}</td>
      <td className="hidden md:table-cell">{item.paymentDate}</td>
      <td className="hidden md:table-cell">{item.receiptNumber || "-"}</td>
      <td>
        <div className="flex items-center gap-2">
          {role === "admin" && (
            <>
              <FormContainer table="feePayment" type="update" data={item} />
              <FormContainer table="feePayment" type="delete" id={item.id} />
            </>
          )}
        </div>
      </td>
    </tr>
  );

  const { page, ...queryParams } = searchParams;

  const p = page ? parseInt(page) : 1;

  const query: Prisma.FeePaymentWhereInput = {};

  if (queryParams) {
    for (const [key, value] of Object.entries(queryParams)) {
      if (value !== undefined) {
        switch (key) {
          case "studentId":
            query.studentId = value;
            break;
          case "feeStructureId":
            query.feeStructureId = parseInt(value);
            break;
          case "status":
            query.status = value;
            break;
          case "search":
            query.OR = [
              { student: { name: { contains: value, mode: "insensitive" } } },
              { student: { surname: { contains: value, mode: "insensitive" } } },
              { feeStructure: { name: { contains: value, mode: "insensitive" } } },
              { receiptNumber: { contains: value, mode: "insensitive" } },
              { transactionId: { contains: value, mode: "insensitive" } },
            ];
            break;
          default:
            break;
        }
      }
    }
  }

  // Role-based filtering
  switch (role) {
    case "admin":
      // Admin can see all payments
      break;
    case "teacher":
      // Teachers should not see payments
      return <div className="p-4">Not authorized to view fee payments.</div>;
    case "student":
      // Students can only see their own payments
      query.studentId = currentUserId!;
      break;
    case "parent":
      // Parents can see payments for their children
      query.student = {
        parentId: currentUserId!
      };
      break;
    default:
      break;
  }

  const [feePaymentsData, count] = await prisma.$transaction([
    prisma.feePayment.findMany({
      where: query,
      include: {
        student: true,
        feeStructure: true,
      },
      take: ITEM_PER_PAGE,
      skip: ITEM_PER_PAGE * (p - 1),
      orderBy: [
        { paymentDate: 'desc' }
      ],
    }),
    prisma.feePayment.count({ where: query }),
  ]);

  // Format date in a readable way
  const formatDate = (date: Date) => {
    return new Date(date).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric'
    });
  };

  // Get payment method and status labels
  const getPaymentMethodLabel = (method: string) => {
    const methodMap: Record<string, string> = {
      'CASH': 'Cash',
      'CARD': 'Card',
      'BANK_TRANSFER': 'Bank Transfer',
      'CHEQUE': 'Cheque',
      'ONLINE': 'Online Payment'
    };
    return methodMap[method] || method;
  };

  const getStatusLabel = (status: string) => {
    const statusMap: Record<string, string> = {
      'PAID': 'Paid',
      'PENDING': 'Pending',
      'FAILED': 'Failed',
      'CANCELLED': 'Cancelled',
      'REFUNDED': 'Refunded'
    };
    return statusMap[status] || status;
  };

  // Transform the data for display
  const data = feePaymentsData.map(payment => ({
    id: payment.id,
    student: `${payment.student.name} ${payment.student.surname}`,
    studentId: payment.studentId,
    feeType: payment.feeStructure.name,
    feeStructureId: payment.feeStructureId,
    amount: new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: 'USD'
    }).format(payment.amount),
    rawAmount: payment.amount,
    paymentDate: formatDate(payment.paymentDate),
    rawPaymentDate: payment.paymentDate,
    paymentMethod: getPaymentMethodLabel(payment.paymentMethod),
    rawPaymentMethod: payment.paymentMethod,
    status: payment.status,
    statusLabel: getStatusLabel(payment.status),
    transactionId: payment.transactionId,
    receiptNumber: payment.receiptNumber,
  }));

  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      {/* TOP */}
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Fee Payments</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSearch />
          <div className="flex items-center gap-4 self-end">
            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-lamaYellow">
              <Image src="/filter.png" alt="" width={14} height={14} />
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-lamaYellow">
              <Image src="/sort.png" alt="" width={14} height={14} />
            </button>
            {role === "admin" && (
              <FormContainer table="feePayment" type="create" />
            )}
          </div>
        </div>
      </div>
      {/* LIST */}
      <Table columns={columns} renderRow={renderRow} data={data} />
      {/* PAGINATION */}
      <Pagination page={p} count={count} />
    </div>
  );
};

export default FeePaymentListPage;
